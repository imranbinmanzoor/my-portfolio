...
<article class="project-case" data-project="${p.slug}">
  ...
  <section class="project-gallery" aria-label="Project preview" data-project-gallery>
    ...
    ${p.views.length>1?'<div id="project-preview" role="tabpanel" aria-labelledby="view-0" tabindex="0"><figure>':'<figure id="project-preview">'}<div class="gallery-stage"><img src="${escape(first.src)}" alt="${escape(first.alt)}" decoding="async" fetchpriority="high" width="1200" height="900" data-preview-image></div><figcaption data-preview-caption>${escape(first.caption)}</figcaption></figure>${p.views.length>1?'</div>':''}
